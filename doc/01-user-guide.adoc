...
Clj-yaml is a Clojure wrapper over https://codeberg.org/snakeyaml/snakeyaml[SnakeYAML].
...
- https://yamlstar.org/[YAMLStar] - A YAML 1.2 load/dump framework for all programming languages.
YAMLStar is a pure Clojure parser, no dependencies on SnakeYAML or other external parsers.
...
SnakeYAML implementation (that clj-yaml uses for low-level encoding and decoding) imposes the default limit of 3 megabyte document size for security reasons (https://codeberg.org/snakeyaml/snakeyaml/issues/547[issue]). If you hit this limitation, you need to explicitly increase the limit by setting the `:code-point-limit` option:
